...
class _AddOrEditProductScreenState extends State<AddOrEditProductScreen> {
  ...
  void initState() {
    ...
    _imageController = TextEditingController(
      text: p != null && (p['images'] as List).isNotEmpty
          ? p['images'][0]['imageUrl']
          : '',
    );
  }
  ...
  Future<void> _submitForm() async {
    ...
    final supplyCategoryId = widget.product?['supplyCategoryId'] ?? 1;

    final productData = {
      if (isEditing) 'productId': productId,
      ...
      'images': [
        {
          'productImageId':
              isEditing && (widget.product!['images'] as List).isNotEmpty
    ...
    try {
      final path = isEditing ? 'Product/$productId' : 'Product';
      final resp = isEditing
          ? await _api.put(path, body: productData)
          : await _api.post(path, body: productData);
      ...
      if (resp.statusCode == 200 || resp.statusCode == 201) {
        ...
        throw Exception(
          'Failed to save product. Status code: ${resp.statusCode}\n${resp.body}',
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
